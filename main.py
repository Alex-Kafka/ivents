...
def start(message):
    ...
    bot.send_message(message.chat.id, f'Привет 👋'
                                      f'\n\nМы рады предложить тебе уникальную возможность подобрать мероприятия, \nкоторые идеально соответствуют твоим интересам! 🎉 \nПросто выбери из нашего списка те темы, которые тебя вдохновляют, и мы подберем для тебя самые захватывающие события📝'
                                      f'\n\nНе упусти шанс открыть для себя что-то новое и интересное! \nКак только закончишь выбор, просто нажми на кнопку "Готово", и мы подготовим для тебя персонализированный список мероприятий 😉 \n\nДавай сделаем твои выходные незабываемыми!🌟', reply_markup=markup)
